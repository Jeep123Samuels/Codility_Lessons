"""Main script for fish_swimming.py"""
...
# If P and Q are two fish and P < Q,
...
def solution(A, B):
    """Returns list of remaining fish."""
    # list for surviving fish so far.
    survived = []
    # list for fish swimming up
    up_fish = []

    for idx, value in enumerate(B):
        # if the fish is swimming upward,
        # ... add fish to up_fish list.
        if value == 1:
            up_fish.append(A[idx])
            continue
        # If downward fish is bigger then max_up_fish, empty up_fish.
        while up_fish and up_fish[-1] < A[idx]:
            up_fish.pop()

        # If no crossing fish, just add to survived.
        if not up_fish:
            survived.append(A[idx])

    survived.extend(up_fish)
    return len(survived)

if __name__ == "__main__":
    # solution([4, 3, 2, 1, 5], [1, 0, 0, 0, 0])
    a = list()
    a.append('a')
    print(dir(a))
    a.append('b')
